# Provides a build environment for Static Linux environments - less features that Debian (no swift support, no emscripten, no GNUStep-Support, no support for cross-compiling to -gnu instead of musl)
...
RUN apk update && apk upgrade && apk add \
    nix nano curl wget gnupg rpm zsh zip git jq pkgconfig \
    make cmake ninja \

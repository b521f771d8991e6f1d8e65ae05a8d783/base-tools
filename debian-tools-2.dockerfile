...
RUN apt update && apt upgrade -y && apt install -y --no-install-recommends curl nix nano wget gpg ssh zsh git git-lfs pkg-config zip unzip tar postgresql postgresql-client \
